...
2. Donor receipt emails and an admin giving report. (Done 2026-06-11 — webhook sends verified donors a receipt; `/api/admin/giving/report` feeds the Giving Stewardship panel in the admin payments tab.)
3. Signed-in visual QA pass on staging (dashboard, admin, family, leader, scholarship, Prayer Companion, Visual Sanctuary).
4. Household/group invite delivery. (Done 2026-06-11 — invitees receive a Resend email with a join link, and pending seats auto-activate once the invited email has an account. Root-cause discovery: the app's email had never delivered at all — the Resend domain `updates.theccndaily.com` failed verification in March because its DNS records were never recreated after the move from Namecheap, senders used the unregistered apex domain, and the stored RESEND_API_KEY carried a UTF-8 BOM that made every API call invalid. All three fixed; first real delivery confirmed in the Resend log. Staging note: the preview environment intentionally has no RESEND_API_KEY, so emails no-op there and send only from production.)
