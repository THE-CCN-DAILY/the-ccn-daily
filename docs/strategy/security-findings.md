...
| C-3 | `/api/auth/profile` sets admin role from **request-body email** (unverified) — privilege escalation; `ON CONFLICT(email)` can clobber the real admin row. | 🔶 Fix: require verified Bearer on the route, derive role from `idTokenEmail`. Needs client to send token on profile call — verify first. |
